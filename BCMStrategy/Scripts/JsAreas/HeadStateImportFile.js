...
$("#importFile").change(function () {
  ...
  else { $("#btnValidateImportFile").hide(); }
});

$("#btnImportFile").click(function () {
  $("#successMessage").html('');
  CommonJS.SetPageLoader(PageLoaderActivity.SHOW);
  var listOfHeadState = lisfOfImportedRecords.filter(function (x) { return x.isValidRecord; });
  $.ajax({
    type: "POST",
    url: BCMConfig.API_HEADSTATEIMPORT_URL,
...
$("#btnValidateImportFile").click(function () {
  ...
  var ajaxRequest = $.ajax({
    type: "POST",
    url: BCMConfig.API_HEADSTATEIMPORTVALIDATE_URL,
    beforeSend: CommonJS.BeforeSendAjaxCall,
    contentType: false,
    processData: false,
    data: data,
